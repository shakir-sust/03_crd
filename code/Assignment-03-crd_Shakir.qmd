...
```{r fitted vs. residual plot: residual independence assumption}
...
```{r fitted vs. residual plot: residual homoscedasticity assumption}
...
```{r interaction means n inside k: to extract the means of nitrogen rates inside each level of potassim rates}
...
```{r interaction means k inside n: to extract the means of potassium rates inside each level of nitrogen rates}
...
ggplot() +
  ...
  geom_boxplot(data = crd_adfw,
               aes(x = krate_kgha, y = yield_kgha,
                   color = nrate_kgha )) + #We want to see the nitrogen rate in the x axis for all levels of potassium rates
  geom_jitter() +
