...
ggplot(crd_dfw, aes(x = nrate_kgha,
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha) + #takes all levels of a specified variable ("potassium rate"/ "krate_kgha" in this case ), then creates separate boxplots for each of the variable levels 
...
#If our goal is to find the best in Nitrogen rates inside each of the Potassium rates, we would run the following
...
#Important note: flipping the order of krate_kgha either before or after the "|" sign makes a differnece
crd_means_kn
```
...
crd_cld_kn <- cld(crd_means_kn, 
                   ...
                   adjust="none",
               Letters=letters) 
#Multiple comparisons for Potassium groups only within the groups of Nitrogen
crd_cld_kn
```
